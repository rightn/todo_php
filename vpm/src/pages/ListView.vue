...
export default {
  ...
  setup() {
    ...
    const toggleTodo = (_obj) => {
      ...
      fetch(`http://rightn.dothome.co.kr/data_update.php?id=${_obj.id}&title=${_obj.title}&body=${_obj.body}&complete=${_obj.complete}`)
      .then(res => res.json())
      .then(data => {
        console.log(data)
      })
...
const searchActive =ref(false);
    ...
    const getTotalSearch = () => {
      ...
      clearTimeout(searchTimer);
      fetch(`http://rightn.dothome.co.kr/data_total_search.php?title=${searchTxt.value}`)
      .then(res => res.json())
      .then(data => {
        // 검색어에 해당하는 데이터 개수
        data_total.value = data.total;
    ...
    const searchTodo = () => {
      console.log('go')
      fetch(`http://rightn.dothome.co.kr/data_read_search.php?page_now=${page_now.value}&data_count=${data_count}&title=${searchTxt.value}`)
      .then(res => res.json())
      .then(data => {
